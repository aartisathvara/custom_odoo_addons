...
class SaleController(http.Controller):
    ...
    def view_sale_order(self, **post):
        if post:
            sale_id = int(post["sale_orders"])
            invoice_add = (
                request.env["sale.order"].browse(int(sale_id)).partner_invoice_id
            )
            invoice_address = (
                invoice_add.street
                + " "
                + (invoice_add.street2 or "")
                + " "
                + invoice_add.city
                + " "
                + (invoice_add.state_id and invoice_add.state_id.name or "")
                + " "
                + (invoice_add.country_id and invoice_add.country_id.name or "")
            )

            delivery_add = (
                request.env["sale.order"].browse(int(sale_id)).partner_shipping_id
            )
            delivery_address = (
                delivery_add.street
                + " "
                + (delivery_add.street2 or "")
                + " "
                + delivery_add.city
                + " "
                + (delivery_add.state_id and delivery_add.state_id.name or "")
                + " "
                + (delivery_add.country_id and delivery_add.country_id.name or "")
            )

            vals = {
                "sale_data": request.env["sale.order"].sudo().browse(sale_id),
                "partner_invoice_address": invoice_address,
                "partner_delivery_address": delivery_address,
            }
            tracking_order_vals = {
                "sales_order_ref": sale_id,
                "user_name": request._uid,
            }
            request.env["tracking.order"].sudo().create(tracking_order_vals)
